...
class MySQLPostRepository extends IPostRepository {
  ...
  async getAll(filters = {}, userId = null) {
    ...
    let query = `
      SELECT p.id, p.title, p.content, p.visibility, p.pinned, p.created_at, p.updated_at,
             p.author_id, p.category_id, p.department_id, p.team_id,
    ...
    const posts = await Promise.all(
      rows.map(async (row) => {
        const [attachments] = await db.query(
          `
        SELECT f.id, f.storage_path, f.original_name, f.mime_type, pa.attachment_type
  ...
  async update(id, updateData) {
    ...
    if (updateData.team_id !== undefined) {
      fields.push("team_id = ?");
      values.push(updateData.team_id);
    }
  ...
  async getCount(filters = {}) {
    ...
    if (filters.department_id) {
      query += ' AND (visibility = "company" OR department_id = ?)';
      params.push(filters.department_id);
    }
...
module.exports = MySQLPostRepository;
